...
public:
    ...
    static IPAddress::ptr Lookup(const std::string& node ,uint32_t port);
    //获取全部IP地址,成功返回true
    static bool Lookup(std::vector<IPAddress::ptr>& vecIPaddr
                ,const std::string& node,uint32_t port);
    //为上面两个集合,节省代码空间
    static IPAddress::ptr Lookup(std::vector<IPAddress::ptr>& vecIPAddr,const std::string& node ,uint32_t port,
...
public:
    ...
    static IPv4Address::ptr Create(const std::string& address ,uint32_t port);
...
public:
    ...
    static IPv6Address::ptr Create(const std::string& address ,uint32_t port);

    IPv6Address(struct sockaddr_in6& addr);
    IPv6Address(const char* addr ,uint32_t port = 0);
